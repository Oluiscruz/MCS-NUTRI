import { ArrowLeft, CircleX, ClipboardClock, FilePlusCorner } from "lucide-react";
...
export default function VisualizarPacientes() {
    ...
    async function fetchAgendamentos() {
        setLoading(true);
        try {
            const response = await axios.get('/api/nutricionista/pacientes-agendados', {
    ...
    async function handleVerFicha(pacienteId) {
        try {
            const response = await axios.get(`/api/nutricionista/ficha/${pacienteId}`);
            ...
            alert('❌ Erro ao carregar ficha do paciente. Tente novamente mais tarde.');
        }
    }
    ...
    async function handleApagarAgendamento(agendamentoId) {
        ...
        try {
            const response = await axios.delete(`/api/nutricionista/agendamento/${agendamentoId}`);
        ...
        <div className="container-viz-pacientes">
            <header className="main-header">
                <div className="infos">
                    ...
                    <p>Acompanhe os agendamentos dos seus pacientes e mantenha-se organizado(a).</p>
                    <em>Cada paciente terá uma ficha de anamnese individual com informações detalhadas.</em>
                </div>
            ...
            <div className="content-pacientes">
                {loading ? (
                    ...
                    agendamentos.map((agendamento) => {
                        console.log('Status do agendamento:', agendamento.status, 'Tipo:', typeof agendamento.status);
                        return (
                            <div key={agendamento.agendamento_id} className="box-paciente">
                                <div className="div-ficha">
                                    ...
                                    <div className="btn-flex">
                                        ...
                                        <button className="ver-ficha"
                                            onClick={() => handleVerFicha(agendamento.paciente_id)}>
                                            <FilePlusCorner size={16} /> Ficha
                                        </button>
                                        <button className="apagar" onClick={() => handleApagarAgendamento(agendamento.agendamento_id)}>
                                            <CircleX size={16} />Apagar
                                        </button>
                                    </div>
                                </div>
                                <div className="infos">

                                    <p><strong>Email:</strong> {agendamento.paciente_email}</p>
                                    <p><strong>Telefone:</strong> {agendamento.paciente_telefone}</p>
                                    <p><strong>Data:</strong> {agendamento.dia}/{agendamento.mes_ano}/{agendamento.ano}</p>
                                    <p><strong>Horário:</strong> {agendamento.hora_agendamento}</p>
                                    <p><strong>Status:</strong>
                                        <select id="select-status"
                                            value={agendamento.status}
                                            onChange={(e) => atualizarStatus(agendamento.agendamento_id, e.target.value)}>
                                            <option value="pendente">Pendente</option>
                                            <option value="confirmado">Confirmado</option>
                                            <option value="cancelado">Cancelado</option>
                                            <option value="concluido">Concluído</option>
                                        </select>
                                        <span className={`status-${agendamento.status}`}>{agendamento.status}</span></p>
                                        ...
                                        <div className="acoes">
                                            <button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    atualizarStatus(agendamento.agendamento_id, 'confirmado');
                                                }}
                                            >
                                                Confirmar
                                            </button>
                                            <button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    atualizarStatus(agendamento.agendamento_id, 'cancelado');
                                                }}
                                            >
                                                Recusar
                                            </button>
                    ...
                    <div className="modal-ficha" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="ficha-content">
                            <div className="info-basica">
                                <p><strong>Email:</strong> {fichaData.paciente_email || 'Não informado'}</p>
                                <p><strong>Telefone:</strong> {fichaData.paciente_telefone || 'Não informado'}</p>
                            </div>
                            <div className="info-anamnese">

                                <p><strong>Altura:</strong> {`${(fichaData.altura)} m`}</p>
                                <p><strong>Peso:</strong> {`${fichaData.peso} kg`}</p>
                                <p><strong>Alergias:</strong> {fichaData.alergias || 'Nenhuma'}</p>
                                <p><strong>Restrições Alimentares:</strong> {fichaData.restricoes_alimentares || 'Nenhuma'}</p>
                                <p><strong>Objetivos:</strong> {fichaData.objetivos || fichaData.objetivo || 'Não informado'}</p>
                            </div>
                        </div>
                        <button className="btn-fechar" onClick={() => setShowFicha(false)}>Fechar</button>
                    </div>
                </div>
            )}
